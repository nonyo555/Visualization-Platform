...
const barChartRace = require('../charts/barChartRace/classBarChartRace')
...
app.post('/barChartRace', async (req, res, next) => {
  var data = [];
  var config;
  var dataFile;
  var configFile;
  ...
  try {
      let visualization = barChartRace.object();
      await visualization.setAttr(data, config)
      let html = visualization.generateHTML();
      fs.writeFileSync('generated/test.html', html, (error) => { console.log(error) });
